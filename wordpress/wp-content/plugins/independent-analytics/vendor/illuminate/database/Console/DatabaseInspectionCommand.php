...
use IAWPSCOPED\Symfony\Component\Process\Exception\ProcessSignaledException;
use IAWPSCOPED\Symfony\Component\Process\Exception\RuntimeException;
...
abstract class DatabaseInspectionCommand extends Command
{
     ...
     * @var array
     */
    protected $typeMappings = ['bit' => 'string', 'citext' => 'string', 'enum' => 'string', 'geometry' => 'string', 'geomcollection' => 'string', 'linestring' => 'string', 'ltree' => 'string', 'multilinestring' => 'string', 'multipoint' => 'string', 'multipolygon' => 'string', 'point' => 'string', 'polygon' => 'string', 'sysname' => 'string'];
    /**
    ...
    {
        $result = $connection->selectOne('SELECT (data_length + index_length) AS size FROM information_schema.TABLES WHERE table_schema = ? AND table_name = ?', [$connection->getDatabaseName(), $table]);
    ...
    {
        $result = $connection->selectOne('SELECT pg_total_relation_size(?) AS size;', [$table]);
        return Arr::wrap((array) $result)['size'];
    }
    /**
    ...
    {
        try {
            $result = $connection->selectOne('SELECT SUM(pgsize) AS size FROM dbstat WHERE name=?', [$table]);
            return Arr::wrap((array) $result)['size'];
    ...
    {
        return \IAWPSCOPED\tap(\interface_exists('IAWPSCOPED\\Doctrine\\DBAL\\Driver'), function ($dependenciesExist) {
    ...
    {
        $command = \IAWPSCOPED\collect($this->composer->findComposer())->push('require doctrine/dbal')->implode(' ');
