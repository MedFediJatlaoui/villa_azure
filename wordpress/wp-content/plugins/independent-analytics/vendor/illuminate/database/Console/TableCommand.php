...
use IAWPSCOPED\Doctrine\DBAL\Schema\ForeignKeyConstraint;
...
class TableCommand extends DatabaseInspectionCommand
{
    ...
    public function handle(ConnectionResolverInterface $connections)
    {
        ...
        $table = $this->argument('table') ?: $this->components->choice('Which table would you like to inspect?', \IAWPSCOPED\collect($schema->listTables())->flatMap(fn(Table $table) => [$table->getName()])->toArray());
        ...
        $data = ['table' => ['name' => $table->getName(), 'columns' => $columns->count(), 'size' => $this->getTableSize($connection, $table->getName())], 'columns' => $columns, 'indexes' => $indexes, 'foreign_keys' => $foreignKeys];
        $this->display($data);
        return 0;
    }
    /**
    ...
    {
        return \IAWPSCOPED\collect($table->getColumns())->map(fn(Column $column) => ['column' => $column->getName(), 'attributes' => $this->getAttributesForColumn($column), 'default' => $column->getDefault(), 'type' => $column->getType()->getName()]);
    }
    /**
    ...
    {
        return \IAWPSCOPED\collect([$column->getAutoincrement() ? 'autoincrement' : null, 'type' => $column->getType()->getName(), $column->getUnsigned() ? 'unsigned' : null, !$column->getNotNull() ? 'nullable' : null])->filter();
    }
    /**
    ...
    {
        return \IAWPSCOPED\collect($table->getIndexes())->map(fn(Index $index) => ['name' => $index->getName(), 'columns' => \IAWPSCOPED\collect($index->getColumns()), 'attributes' => $this->getAttributesForIndex($index)]);
    }
    /**
    ...
    {
        return \IAWPSCOPED\collect(['compound' => \count($index->getColumns()) > 1, 'unique' => $index->isUnique(), 'primary' => $index->isPrimary()])->filter()->keys()->map(fn($attribute) => Str::lower($attribute));
    }
    /**
    ...
    protected function displayForCli(array $data)
    {
        ...
        if ($size = $table['size']) {
            $this->components->twoColumnDetail('Size', \number_format($size / 1024 / 1024, 2) . 'MiB');
        }
        $this->newLine();
        if ($columns->isNotEmpty()) {
            $this->components->twoColumnDetail('<fg=green;options=bold>Column</>', 'Type');
            $columns->each(function ($column) {
                $this->components->twoColumnDetail($column['column'] . ' <fg=gray>' . $column['attributes']->implode(', ') . '</>', ($column['default'] ? '<fg=gray>' . $column['default'] . '</> ' : '') . '' . $column['type'] . '');
            });
            $this->newLine();
        }
        ...
        if ($foreignKeys->isNotEmpty()) {
            ...
            $foreignKeys->each(function ($foreignKey) {
                $this->components->twoColumnDetail($foreignKey['name'] . ' <fg=gray;options=bold>' . $foreignKey['local_columns']->implode(', ') . ' references ' . $foreignKey['foreign_columns']->implode(', ') . ' on ' . $foreignKey['foreign_table'] . '</>', $foreignKey['on_update'] . ' / ' . $foreignKey['on_delete']);
            });
            $this->newLine();
        }
    }
