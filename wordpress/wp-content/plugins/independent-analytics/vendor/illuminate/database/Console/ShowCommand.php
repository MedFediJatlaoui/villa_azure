...
use IAWPSCOPED\Doctrine\DBAL\Schema\AbstractSchemaManager;
use IAWPSCOPED\Doctrine\DBAL\Schema\Table;
use IAWPSCOPED\Doctrine\DBAL\Schema\View;
...
class ShowCommand extends DatabaseInspectionCommand
{
    ...
    public function handle(ConnectionResolverInterface $connections)
    {
        ...
        $data = ['platform' => ['config' => $this->getConfigFromDatabase($database), 'name' => $this->getPlatformName($schema->getDatabasePlatform(), $database), 'open_connections' => $this->getConnectionCount($connection)], 'tables' => $this->tables($connection, $schema)];
    ...
    {
        return \IAWPSCOPED\collect($schema->listTables())->map(fn(Table $table, $index) => ['table' => $table->getName(), 'size' => $this->getTableSize($connection, $table->getName()), 'rows' => $this->option('counts') ? $connection->table($table->getName())->count() : null, 'engine' => rescue(fn() => $table->getOption('engine'), null, \false), 'comment' => $table->getComment()]);
    }
    /**
    ...
    {
        return \IAWPSCOPED\collect($schema->listViews())->reject(fn(View $view) => str($view->getName())->startsWith(['pg_catalog', 'information_schema', 'spt_']))->map(fn(View $view) => ['view' => $view->getName(), 'rows' => $connection->table($view->getName())->count()]);
    }
    /**
    ...
    protected function displayForCli(array $data)
    {
        ...
        if ($tableSizeSum = $tables->sum('size')) {
            $this->components->twoColumnDetail('Total Size', \number_format($tableSizeSum / 1024 / 1024, 2) . 'MiB');
        }
        $this->newLine();
        if ($tables->isNotEmpty()) {
            ...
            $tables->each(function ($table) {
                ...
                $this->components->twoColumnDetail($table['table'] . ($this->output->isVerbose() ? ' <fg=gray>' . $table['engine'] . '</>' : null), ($tableSize ? $tableSize : '—') . ($this->option('counts') ? ' <fg=gray;options=bold>/</> <fg=yellow;options=bold>' . \number_format($table['rows']) . '</>' : ''));
        ...
        if ($views && $views->isNotEmpty()) {
            ...
            $views->each(fn($view) => $this->components->twoColumnDetail($view['view'], \number_format($view['rows'])));
            $this->newLine();
        }
    }
}
