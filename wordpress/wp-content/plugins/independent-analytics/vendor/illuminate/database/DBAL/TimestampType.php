...
use IAWPSCOPED\Doctrine\DBAL\Exception as DBALException;
...
class TimestampType extends Type implements PhpDateTimeMappingType
{
    ...
    public function getSQLDeclaration(array $fieldDeclaration, AbstractPlatform $platform)
    {
        return match ($name = $platform->getName()) {
            'mysql', 'mysql2' => $this->getMySqlPlatformSQLDeclaration($fieldDeclaration),
            'postgresql', 'pgsql', 'postgres' => $this->getPostgresPlatformSQLDeclaration($fieldDeclaration),
            'mssql' => $this->getSqlServerPlatformSQLDeclaration($fieldDeclaration),
            'sqlite', 'sqlite3' => $this->getSQLitePlatformSQLDeclaration($fieldDeclaration),
