...
trait EnumeratesValues
{
    ...
    public function value($key, $default = null)
    {
        if ($value = $this->firstWhere($key)) {
            return \IAWPSCOPED\data_get($value, $key, $default);
        }
    ...
    public function whereIn($key, $values, $strict = \false)
    {
        ...
        return $this->filter(fn($item) => \in_array(\IAWPSCOPED\data_get($item, $key), $values, $strict));
    }
    /**
     * Filter items by the given key value pair using strict comparison.
     *
    ...
    {
        return $this->filter(fn($item) => \IAWPSCOPED\data_get($item, $key) < \reset($values) || \IAWPSCOPED\data_get($item, $key) > \end($values));
    }
    /**
     * Filter items by the given key value pair.
     *
    ...
    {
        $values = $this->getArrayableItems($values);
        return $this->reject(fn($item) => \in_array(\IAWPSCOPED\data_get($item, $key), $values, $strict));
    }
    /**
     * Filter items by the given key value pair using strict comparison.
     *
    ...
    protected function valueRetriever($value)
    {
        ...
        return fn($item) => \IAWPSCOPED\data_get($item, $value);
    }
    /**
