...
class NotificationFake implements NotificationDispatcher, NotificationFactory
{
    ...
    {
        $actualCount = \IAWPSCOPED\collect($this->notifications)->flatten(1)->reduce(fn($count, $sent) => $count + \count($sent[$notification] ?? []), 0);
    ...
    {
        $actualCount = \IAWPSCOPED\collect($this->notifications)->flatten(3)->count();
