...
class Benchmark
{
    ...
    {
        return \IAWPSCOPED\collect(Arr::wrap($benchmarkables))->map(function ($callback) use($iterations) {
            return \IAWPSCOPED\collect(\range(1, $iterations))->map(function () use($callback) {
                \gc_collect_cycles();
                $start = \hrtime(\true);
                $callback();
    ...
    {
        $result = \IAWPSCOPED\collect(static::measure(Arr::wrap($benchmarkables), $iterations))->map(fn($average) => \number_format($average, 3) . 'ms')->when($benchmarkables instanceof Closure, fn($c) => $c->first(), fn($c) => $c->all());
        dd($result);
    }
}
