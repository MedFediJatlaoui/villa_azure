...
final class ASCII
{
     ...
     */
    private const UNKNOWN_TRANSLITERATION_MARKERS = ['[?]' => \true, '[?] ' => \true];
    /**
     ...
     */
    public static function clean(string $str, bool $normalize_whitespace = \true, bool $keep_non_breaking_space = \false, bool $normalize_msword = \true, bool $remove_invisible_characters = \true, bool $remove_invalid_utf8 = \true) : string
    {
    ...
    public static function to_ascii(string $str, string $language = self::ENGLISH_LANGUAGE_CODE, bool $remove_unsupported_chars = \true, bool $replace_extra_symbols = \false, bool $use_transliterate = \false, bool $replace_single_chars_only = \false) : string
    {
        ...
        if (!$replace_extra_symbols && !\preg_match('/' . self::REGEX_PRINTABLE_ASCII . '/', $str)) {
            return $str;
        }
        $language = self::get_language($language);
        /** @phpstan-var ASCII::*_LANGUAGE_CODE $language - hack for phpstan */
        if (!$replace_extra_symbols && \strlen($str) <= 64) {
            $isValidUtf8 = \true;
        ...
        if (!$replace_extra_symbols && !\preg_match('/[\\x80-\\xFF]/', $str)) {
            if ($remove_unsupported_chars) {
                $str = (string) \str_replace(["\r\n", "\n", "\r", "\t"], ' ', $str);
                $str = (string) \preg_replace('/' . self::$REGEX_ASCII . '/', '', $str);
            }
        ...
        if ($use_transliterate && !$replace_extra_symbols && !$replace_single_chars_only && $language === self::ENGLISH_LANGUAGE_CODE) {
            $str = self::to_transliterate($str, null, \false);
        } else {
    ...
    public static function to_slugify(string $str, string $separator = '-', string $language = self::ENGLISH_LANGUAGE_CODE, array $replacements = [], bool $replace_extra_symbols = \false, bool $use_str_to_lower = \true, bool $use_transliterate = \false) : string
    {
        ...
        if (!$replace_extra_symbols && !$use_transliterate && self::get_language($language) === self::ENGLISH_LANGUAGE_CODE && !\preg_match('/' . self::REGEX_PRINTABLE_ASCII . '/', $str)) {
        ...
        if ($use_str_to_lower) {
            $str = \strtolower($str);
            $str = (string) \preg_replace('/[^a-z\\d\\s\\-_' . \preg_quote($separator, '/') . ']/', '', $str);
        } else {
    ...
    public static function to_transliterate(string $str, $unknown = '?', bool $strict = \false) : string
    {
        ...
        if (isset($str[63]) && !\preg_match('/' . self::REGEX_PRINTABLE_ASCII . '/', $str)) {
            return $str;
        }
        ...
        $unknownCacheKey = $unknown === null ? "\x00null" : "\x01" . $unknown;
        if ($SUPPORT_INTL === null) {
            $SUPPORT_INTL = \extension_loaded('intl');
        }
        $warmPathAlreadyApplied = \false;
        if ($unknown !== '?' && isset($WARM_MAPS[$unknownCacheKey]) && \preg_match('//u', $str) === 1) {
            $warmStr = \strtr($str, $WARM_MAPS[$unknownCacheKey]);
            if (!\preg_match('/[\\x80-\\xFF\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F\\x7F]/', $warmStr)) {
                return $warmStr;
            }
            $str = $warmStr;
            $warmPathAlreadyApplied = \true;
        }
        // only run the heavy clean() regex when the string has invalid UTF-8
        if (\preg_match('//u', $str) === 1) {
            if ($unknown === '?' || \strpos($str, "\xc2") !== \false || \strpos($str, "\xe2") !== \false || \preg_match('/[\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F\\x7F]/', $str) === 1) {
                $str_before_clean = $str;
                $str = self::normalize_whitespace($str);
                $str = self::normalize_msword($str);
                $str = self::remove_invisible_characters($str);
                $str = self::clean($str, \true, \false, \true, \false);
                if ($str !== $str_before_clean && \preg_match('/' . self::$REGEX_ASCII . '/', $str) === 0) {
                    return $str;
                }
            }
        } else {
        ...
        if ($strict && $SUPPORT_INTL === \true) {
            ...
            if ($str_tmp !== \false) {
                if ($str_tmp !== $str && \preg_match('/' . self::$REGEX_ASCII . '/', $str_tmp) === 0) {
                    return $str_tmp;
                }
                $str = $str_tmp;
            }
        ...
        if (!$warmPathAlreadyApplied && isset($WARM_MAPS[$unknownCacheKey])) {
        ...
        if (\preg_match_all(self::UTF8_MULTIBYTE_SEQUENCE_RX, $str, $nonAsciiMatches)) {
            ...
            foreach ($nonAsciiMatches[0] as $c) {
                ...
                if (!\array_key_exists($c, $TRANSLIT_CHAR_CACHE)) {
                    ...
                    $bankPos = $ord & 255;
                    if (isset($UTF8_TO_TRANSLIT[$bank][$bankPos]) && !isset(self::UNKNOWN_TRANSLITERATION_MARKERS[$UTF8_TO_TRANSLIT[$bank][$bankPos]])) {
     ...
     */
    private static function to_ascii_replace(string $str, string $language, bool $replace_extra_symbols, bool $replace_single_chars_only, ?bool &$isValidUtf8 = null) : string
    {
        ...
        if (!$replace_extra_symbols && \strlen($str) <= 64) {
            ...
            $filteredMap = [];
            if (!$replace_single_chars_only && $charCount >= 2) {
     ...
     * @return array<string, string>
     */
    private static function getAsciiAllReplacementMap(bool $replace_extra_symbols, bool $replace_single_chars_only) : array
    {
        static $CACHE = [];
        ...
        $CACHE[$cacheKey] = self::filterAsciiReplacementMap(self::charsArrayWithSingleLanguageValues($replace_extra_symbols, \false), $replace_single_chars_only);
        return $CACHE[$cacheKey];
    }
    /**
     * @phpstan-param ASCII::*_LANGUAGE_CODE $language
     ...
     * @return array<string, string>
     */
    private static function getAsciiLanguageReplacementMap(string $language, bool $replace_extra_symbols, bool $replace_single_chars_only) : array
    {
        static $CACHE = [];
        $cacheKey = $language . '-' . (int) $replace_extra_symbols . '-' . (int) $replace_single_chars_only;
        if (isset($CACHE[$cacheKey])) {
        ...
        $CACHE[$cacheKey] = self::filterAsciiReplacementMap(self::charsArrayWithOneLanguage($language, $replace_extra_symbols, \false), $replace_single_chars_only);
        return $CACHE[$cacheKey];
    }
    /**
    ...
    private static function filterAsciiReplacementMap(array $map, bool $replace_single_chars_only) : array
    {
        ...
        foreach ($map as $char => $replacement) {
            ...
            if (isset($char[4]) || \preg_match('/^.$/us', $char) !== 1) {
                unset($map[$char]);
            }
        }
        return $map;
