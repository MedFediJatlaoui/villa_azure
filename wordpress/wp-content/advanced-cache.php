...
// WP-Optimize advanced-cache.php (written by version: 4.6.0) (homeurl: https://hotelvillaazur.com/) (abspath: /var/www/html/) (do not change this line, it is used for correctness checks)
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://hotelvillaazur.com/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-hotelvillaazur.com.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-hotelvillaazur.com.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-hotelvillaazur.com.php');
}
