...
(function () {
  ...
  var LANG = window.VAZ_RESA.lang || 'fr';
  var CAL  = window.VAZ_RESA.calendar;
  var WORDS = window.VAZ_RESA.words || {};
  var POP  = '#pum-' + CFG.popup_id;
  var FORM = '#fluentform_' + CFG.form_id;

  // Mirrors VAZ_I18N::plural() in PHP — same WORDS table, shared via
  // wp_localize_script, so the two can never drift out of sync.
  function word(key, n) {
    var forms = (WORDS[key] && (WORDS[key][LANG] || WORDS[key].fr)) || [key, key];
    n = Math.abs(n);
    if ((LANG === 'pl' || LANG === 'ru') && forms.length === 3) {
  ...
  // sprintf-lite: replaces %d/%s placeholders in order (matches the PHP
  // templates in VAZ_I18N, e.g. "maximum %d %s").
  function fmt(template) {
    var args = Array.prototype.slice.call(arguments, 1);
    var i = 0;
    return String(template).replace(/%[ds]/g, function () { return args[i++]; });
  }
  ...
  function paintSummary($popup, q, inIso, outIso) {
    ...
    var roomsHtml = q.rooms.map(function (r) {
      ...
      var occ = r.adults + ' ' + word('adult', r.adults);
      if (r.children > 0) { occ += ', ' + r.children + ' ' + word('child', r.children); }
      ...
      if (r.single_supp > 0)    { notes.push(T.singleSupp + ' · +' + amount(r.single_supp)); }
      ...
      return '<div class="vaz-sum-room">' +
        ...
        '<div class="vaz-sum-room-meta">' + boardLabel + ' · ' + occ + ' · ' +
          q.nights + ' ' + word('night', q.nights) + '</div>' +
        notesHtml +
      '</div>';
    }).join('');
    ...
    var taxDetail = fmt(T.touristTaxNote, tx.per_person_per_night, q.currency, tx.max_nights, word('night', tx.max_nights));
    ...
    $s.html('' +
      ...
      '<div class="vaz-sum-rows">' +
        line(T.accommodation, amount(accSubtotal)) +
  ...
  // "10 août → 13 août 2026 · 3 nuits" — locale-aware (CAL.locale, e.g. "de-DE").
  function formatStay(inIso, outIso, nights) {
    var locale = (CAL && CAL.locale) || 'fr-FR';
    var a = engine._parse(inIso).toLocaleDateString(locale, { day: 'numeric', month: 'long' });
    var b = engine._parse(outIso).toLocaleDateString(locale, { day: 'numeric', month: 'long', year: 'numeric' });
    return a + ' → ' + b + ' · ' + nights + ' ' + word('night', nights);
  }
  ...
  function syncHidden($popup, q) {
    ...
    var summary = q.rooms.map(function (r) {
      ...
      var occ   = r.adults + ' ' + word('adult', r.adults);
      if (r.children > 0) { occ += ' + ' + r.children + ' ' + word('child', r.children); }
      return T.room + ' ' + r.index + ' — ' + type + ' · ' + board + ' · ' + occ +
        ' — ' + amount(r.subtotal);
    }).join('\n');
    ...
    var bd = [];
    if (q.nights) {
      bd.push(T.stay + ' : ' + q.nights + ' ' + word('night', q.nights));
      bd.push(T.accommodation + ' : ' + amount(q.accommodation));
      if (q.child_discount > 0) { bd.push(T.childDiscount + ' : -' + amount(q.child_discount)); }
      if (q.adult_discount > 0) { bd.push(T.adultDiscount + ' (-30%) : -' + amount(q.adult_discount)); }
      if (q.single_supp > 0)    { bd.push(T.singleSupp + ' : ' + amount(q.single_supp)); }
      bd.push(T.touristTax + ' : ' + amount(q.tourist_tax));
      bd.push(T.total.toUpperCase() + ' : ' + amount(q.total));
    }
  ...
  // The Fluent Forms AJAX submission carries no /en/, /de/... URL prefix, so
  // pll_current_language() can't be trusted server-side at submit time. This
  // field captures the language the guest was actually reading (CFG was
  // localized at page-render time, when Polylang's detection IS accurate),
  // the same technique already used for vaz_hp/vaz_ts in villa-azur-fix.php.
  function ensureLangField($form) {
